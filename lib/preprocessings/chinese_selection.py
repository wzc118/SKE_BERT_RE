...
class Chinese_selection_preprocessing(object):
    ...
    def _read_line(self, line: str) -> Optional[str]:
        ...
        if 'spo_list' in instance:
            ...
            position = self.spo_to_se(text,spo_list)
    ...
    def spo_to_se(self, text, spo_list):
        # p: {s,o}
        selection = []
        for triplet in spo_list:
            ...
            object = triplet['object']
            subject = triplet['subject']
            predicate = self.relation_vocab[triplet['predicate']]

            o_s = text.find(object)
            o_e = o_s + len(object) - 1

            s_s = text.find(subject)
            s_e = s_s + len(subject) - 1
